...
struct ColorView: View {
    ...
    private let desWidth: CGFloat = 100
    ...
    var body: some View {
        ...
        Section("Start Color") {
            SliderBuilder(description: "Hue", descWidth: desWidth, value: $vm.startHue) {
                Slider(value: $vm.startHue, in: 0 ... 360, step: 1)
            }
            SliderBuilder(description: "Saturation", descWidth: desWidth, value: $vm.startSaturation) {
                Slider(value: $vm.startSaturation, in: 0 ... 360, step: 1)
            }
            SliderBuilder(description: "Brightness", descWidth: desWidth, value: $vm.startBrightness) {
                Slider(value: $vm.startBrightness, in: 0 ... 360, step: 1)
            }
        }
        Section("New Branch Color Adjustment") {
            SliderBuilder(description: "Hue", descWidth: desWidth, value: $vm.hueChange) {
                Slider(value: $vm.hueChange, in: -180 ... 180, step: 1)
            }
            SliderBuilder(description: "Saturation", descWidth: desWidth, value: $vm.saturationChange) {
                Slider(value: $vm.saturationChange, in: -180 ... 180, step: 1)
            }
            SliderBuilder(description: "Brightness", descWidth: desWidth, value: $vm.brightnessChange) {
